...
### Signal Inbound Dead — WebSocket Client Contending With Webhook (Root Cause of "Not Responding Ever")
- **Symptom**: Bot went dark for 60–90min at a time, then briefly responded after the signal-watchdog restarted signal-api, then went dark again. Owner got constant "offline" alerts.
- **Ground-truth diagnosis** (from `/var/log/signal-cli-json-rpc-1/out.log` inside signal-api): the signal-cli daemon **receives every inbound message perfectly** (test message "hey" from Karen was in the daemon log with a full `dataMessage`). Send also works (POST /v2/send → 201). So neither receive-from-Signal nor send was broken.
- **Actual root cause**: the bot ran **two competing inbound consumers at once** — the `RECEIVE_WEBHOOK_URL` webhook (`/signal/webhook`) AND a WebSocket client on `ws://signal-api:8080/v1/receive/{number}`. `SIGNAL_USE_WEBHOOK=true` was set, but `SignalAdapter.start()` **ignored the `this.useWebhook` flag** and unconditionally called `_startWebSocket()`. In MODE=json-rpc the daemon delivers each envelope to a single consumer, so the WS client starved the webhook forwarder — inbound only trickled through in the gaps when the WS was disconnected (hence the 400s: `GET /v1/receive → 400` in json-rpc mode) or after a watchdog restart.
- **The 2026-06-19 note below was WRONG**: the webhook DOES forward in v0.100 json-rpc (verified live — the webhook delivered "hey" → "heyyy 💋"). Adding the WS client didn't fix inbound; it *caused* the contention.
- **Fix**: `start()` now honors `this.useWebhook` — when true (the default), it does NOT start the WS client and relies solely on the webhook. Single consumer, no contention. `_poll()` is dead code (never bootstrapped); `_startWebSocket()` now has exactly one caller (the `useWebhook=false`/MODE=normal branch).
- **File changed**: `claude-api/adapters/signal.js` (`start()` receive-mode branch).
- **Why the watchdog is already aligned**: `signal-watchdog.js` keys off webhook-envelope activity (`recordWebhookActivity`), not the WS client, so with contention gone the webhook receives continuously (receipts flow every few min) and the 60-min restart loop stops. The `/health` `signal_ws` field is mislabeled "ws" but actually reflects webhook activity.

### Self-Repair Hardening — "when it self-repairs it breaks forever" (2026-07-06)
Owner reported that automated recovery destabilizes a healthy bot / never converges. A 4-way parallel audit found the culprits; all fixed and independently reviewed (no blocking bugs). Each fix makes a repair action CONVERGE instead of looping, and stops recovery from firing on a healthy system.
- **signal-watchdog.js**: (1) `lastWebhookAt` is now seeded from persisted state / boot time instead of `0`, so a freshly-booted bot in a quiet period isn't judged "dead". (2) `STALE_WEBSOCKET_MS` 60→90min (a quiet chat legitimately produces zero envelopes; silence ≠ death). (3) `lastStaleRestartAt` is now persisted+restored alongside `staleRestartCount` — previously the count persisted but the timestamp didn't, so a restarted bot saw `lastStaleRestartAt=0`, bypassed the cooldown, and immediately fired an escalated force-recreate; this also re-enables the 2h decay. (4) stale-check simplified to require BOTH uptime>window AND staleness>window.
- **oncall-watchdog.js**: (1) **Event-loop-lag restart** was `process.exit(1)` with an in-memory counter — combined with auto-resume relaunching the same heavy task, it restarted every ~6min forever. Now capped at `MAX_LAG_RESTARTS=2` per rolling hour via a PERSISTED ledger (`/app/data/oncall-watchdog-state.json`), then falls back to escalate-only. (2) **Tunnel revive** defeated the tunnel's own circuit breaker (revived a dead tunnel every 2min forever). Now bounded to `MAX_TUNNEL_REVIVES=3`, resets on health, escalates when exhausted. (3) **Process-leak killer** killed the OLDEST claude procs whenever count>10 — which destroyed legitimate long-running sessions under normal concurrent load. Now kills ONLY procs older than `LEAK_AGE_MS=100min` (above the 90min hard session cap = provably orphaned); a high count of young procs escalates instead of killing possibly-active work. (4) The lag check's `setTimeout` body is wrapped in try/catch that always resolves, and the state read is `Array.isArray`-guarded, so a corrupt state file can't hang the whole watchdog (`_running` stuck true → all cycles skipped).
- **watchdog.sh**: (1) added a `flock` run-lock (cron + wsl-autostart.bat could run concurrently through the down/prune/rebuild block). (2) the rebuild path no longer does `down` + `docker system prune -af` BEFORE building — that deleted the last-good image, so a failed build left nothing to fall back to (permanent outage). Now `up -d --build` in place; `docker image prune -f` only AFTER a confirmed-running container. (3) the single `timeout 10 docker info → exit 2` (which makes the caller run `wsl --shutdown`) is now a 3×20s retry loop, so a normal slow-under-load `docker info` no longer triggers a shutdown.
- **wsl-autostart.bat**: (1) fast-path "is the bot running" check retries 3× with a 20s timeout before concluding it's down — a slow-but-healthy `docker inspect` no longer drags a healthy bot into recovery (the on-ramp to the death spiral). (2) new `:wsl_shutdown` subroutine refuses to `wsl --shutdown` again within a 10-min cooldown (stamped in `%USERPROFILE%\mybot-wsl-shutdown-cooldown.txt`); all four recovery-path shutdowns route through it. This breaks the shutdown→cold-boot(slower)→shutdown-again-every-minute spiral. Recovery still makes progress when a shutdown is suppressed (boot attempts are unconditional), so no deadlock.
- **scripts/mybot-heartbeat.ps1**: (1) `/health` curl timeout 15s→35s so the external monitor is LOOSER than the container's own 30s healthcheck — a busy-but-healthy bot (mid Claude session) is no longer flagged down and restarted, killing the session. (2) retry now fires on ANY failure including a timeout (the old code skipped retry on timeout — the case that most needs it). (3) HCS path primes the failure counter to `MaxFailures-2` (not max) so a single transient HCS blip + one slow check can't instantly restart. (4) disk-emergency cleanup gets a 15-min cooldown file so it doesn't relaunch aggressive `Remove-Item -Recurse` storms every 30s tick.
- **Known residual (bounded, not a loop)**: a truly silent Signal chat (zero envelopes incl. receipts/typing for 90min) will still trigger up to ~5 tiered signal-api restarts over 2-3h before settling to hourly-alert. Ambient traffic (receipts/presence) normally prevents this; the root WS-contention fix keeps the receive path alive so envelopes actually flow. Future improvement: an active receive-liveness probe instead of inferring from envelope silence.

### WebSocket Receive Mode (2026-06-19 — SUPERSEDED, diagnosis was incorrect)
- **Claimed root cause (WRONG)**: `bbernhard/signal-cli-rest-api` v0.100 in json-rpc mode does NOT forward envelopes to `RECEIVE_WEBHOOK_URL`. — In fact it does; see the 2026-07-06 entry above.
- **What it did**: Added an active WebSocket client (`ws` package) connecting to `ws://signal-api:8080/v1/receive/{number}`. This became the *cause* of the contention fixed on 2026-07-06.
...
## Next Steps
- **Active Signal receive-liveness probe** (replaces envelope-silence heuristic): One honest caveat remains after the 2026-07-06 self-repair hardening — a truly silent Signal chat (zero traffic incl. receipts for 90min) can still cause a few *bounded* signal-api restarts before settling to alert-only. It's no longer an infinite loop, and the WS-contention fix keeps the receive path alive so envelopes actually flow. The clean long-term fix is an **active receive-liveness probe** (deterministically verify the signal-cli daemon's receive connection is up) instead of inferring death from envelope silence. Until then, `signal-watchdog.js` stale-restart is a bounded last resort, not a reliable liveness signal.
